...
stream: `/directory/events` ingestion also fans applied L2 batches out
to the usage/eviction consumers (`CacheEventRouter`), replacing the
legacy `/quota/events` path.
