...
sequence it was stored under. It cannot be derived: chunk hashes are
...
- **key → tokens**: `key.chunk_hash` → binding
  (`get_token_bindings(chunk_hashes)`, served by `POST
  /directory/lookup`'s keys form, which returns the tokens and the
  offset).
...
- **fragment tokens → keys** (blend-style): rolling
  ...
  [content_index.md](content_index.md), served by `POST
