...
Code: `lmcache/v1/mp_coordinator/content_index.py`,
`lmcache/v1/mp_coordinator/blend_client.py`,
`lmcache/v1/multiprocess/modules/blend_v3.py`.
Index internals: [content_index.md](content_index.md).
...
content index is a derived view of the key directory's token bindings,
...
STORE (blend server, worker-0)
  ...
  storage layer emits l1/l2 store events
        ── POST /directory/events ──▶ key directory: token bindings
                                        └─▶ content index: fingerprints

LOOKUP (blend server, cb_unified_lookup)
  local matcher (always) ─┐
  request tokens          │
    ── POST /directory/blend-lookup ──▶ content index: roll + filter +
