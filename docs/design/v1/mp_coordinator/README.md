...
lmcache/v1/mp_coordinator/
  ...
  content_index.py      # ContentIndex: fragment (blend) lookup over those bindings
...
## Fleet CacheBlend lookup (`content_index.py`)

Cross-request, cross-instance blend reuse is served by the **content index**,
...
`BLEND_PROBE_STRIDE`. See [content_index.md](content_index.md).
...
[content_index.md](content_index.md) for what changed and why.
