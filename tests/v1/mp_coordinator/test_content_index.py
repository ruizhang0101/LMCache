...
from lmcache.v1.mp_coordinator.content_index import ContentIndex
...
def _index(probe_stride: int = 1) -> ContentIndex:
    return ContentIndex(chunk_size=CHUNK, probe_stride=probe_stride)
...
def test_invalid_construction_is_rejected(chunk_size: int, stride: int):
    with pytest.raises(ValueError):
        ContentIndex(chunk_size=chunk_size, probe_stride=stride)
...
def test_matches_a_brute_force_scan_on_random_input():
    ...
    chunk = 8
    index = ContentIndex(chunk_size=chunk, probe_stride=1)
...
def _directory() -> KeyDirectory:
    return KeyDirectory(chunk_size=CHUNK, probe_stride=1)
...
def test_a_stored_chunk_becomes_fragment_matchable():
    ...
    matches = directory.match_content(np.asarray([0, 10, 11, 12, 13], dtype=np.uint64))
...
def test_deleting_the_last_placement_removes_it_from_fragment_matching():
    ...
    assert directory.match_content(np.asarray([10, 11, 12, 13], dtype=np.uint64)) == []
    assert directory.content_stats().num_contents == 0
...
def test_chunk_stays_matchable_while_any_rank_holds_it():
    ...
    assert _tuples(directory.match_content(query)) == [(_key(1).chunk_hash, 0, 0)]

    directory.apply_batch(
        _store([_key(1, kv_rank=1)], [], seq=3, event_type=CacheEventType.DELETE)
    )
    assert directory.match_content(query) == []
...
def test_restoring_a_chunk_with_new_content_retires_the_old_fingerprint():
    ...
    assert directory.match_content(np.asarray([10, 11, 12, 13], dtype=np.uint64)) == []
    assert _tuples(
        directory.match_content(np.asarray([20, 21, 22, 23], dtype=np.uint64))
    ) == [(_key(1).chunk_hash, 0, 0)]
    assert directory.content_stats().num_contents == 1
...
def test_chunk_with_unreported_offset_is_not_fragment_matchable():
    ...
    binding = directory.get_token_bindings([_key(1).chunk_hash])[0]
    assert binding.token_ids.tolist() == [10, 11, 12, 13]  # content known
    assert binding.token_offset == UNKNOWN_TOKEN_OFFSET
    assert directory.content_stats().num_chunks == 0  # but not indexed
    assert directory.match_content(np.asarray([10, 11, 12, 13], dtype=np.uint64)) == []
...
def test_a_later_offset_bearing_store_makes_the_chunk_matchable():
    ...
    assert directory.content_stats().num_chunks == 0

    directory.apply_batch(_store([_key(1)], [10, 11, 12, 13], seq=2, token_offset=256))

    matches = directory.match_content(np.asarray([10, 11, 12, 13], dtype=np.uint64))
