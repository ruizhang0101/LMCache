...
def test_l2_listener_maps_callbacks_to_events():
    sink = _RecordingSink()
    emitter = _emitter(sink)
    listener = L2DirectoryListener(emitter, backend="fs")
...
def test_l2_listener_rejects_mismatched_sizes():
    listener = L2DirectoryListener(_emitter(_RecordingSink()), backend="fs")
...
def test_l1_listener_maps_callbacks_to_events():
    sink = _RecordingSink()
    emitter = _emitter(sink)
    listener = L1DirectoryListener(emitter, backend="dram")

    listener.on_l1_keys_write_finished([_key(1), _key(2)], [100, 200])
    listener.on_l1_keys_finish_write_and_reserve_read([_key(3)], [300])
    listener.on_l1_keys_read_finished([_key(1)])
    listener.on_l1_keys_accessed([_key(2)])
    listener.on_l1_keys_deleted_by_manager([_key(3)])
    asyncio.run(emitter.flush())
...
def test_l1_listener_rejects_mismatched_sizes():
    listener = L1DirectoryListener(_emitter(_RecordingSink()), backend="dram")
    with pytest.raises(ValueError):
        listener.on_l1_keys_write_finished([_key(1), _key(2)], [100])


def test_l1_backend_name_by_medium():
    memory_config = L1MemoryManagerConfig(size_in_bytes=1 << 20, use_lazy=False)
    assert l1_backend_name(L1ManagerConfig(memory_config=memory_config)) == "dram"
    ...
    assert l1_backend_name(L1ManagerConfig(memory_config=devdax_config)) == "devdax"
