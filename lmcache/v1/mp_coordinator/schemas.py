...
class DirectoryKeyPlacements(BaseModel):
    ...
    Attributes:
        ...
        token_offset: Token position of the chunk's first token in the
            sequence it was stored under; ``0`` when ``token_ids`` is
            empty.
    """
...
class BlendLookupRequest(BaseModel):
    ...
    Unlike ``/directory/lookup``, the query need not be a prefix: the
    coordinator finds cached chunk content at any offset within it.

    Attributes:
        tokens_b64: The query tokens, packed via :func:`encode_tokens`
            (base64 little-endian ``uint32``) — ~1.4x smaller than a
            JSON list and decoded in one ``np.frombuffer``.
    """
    ...
    def _validate_tokens_b64(cls, value: str) -> str:
        ...
        Decoding inside the handler would surface bad client input as a
        500; validating here returns 422.
...
class BlendMatchModel(BaseModel):
    """Wire form of one content match (see ``ContentMatch``).

    Attributes:
        chunk_hash: Hex of the matched chunk's ``ObjectKey.chunk_hash``.
            The caller expands it with its own model, salt, and world
            size.
        old_st: The chunk's position in the sequence it was stored under
            (re-RoPE source).
        cur_st: The position in the query where it was found (re-RoPE
            target).
    """
...
class BlendLookupResponse(BaseModel):
    ...
    Attributes:
        matches: Matched chunks, ascending by ``cur_st``. Matches may
