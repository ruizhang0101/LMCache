...
Storage listeners (:class:`L1DirectoryListener`, per-adapter
:class:`L2DirectoryListener`) feed a :class:`CacheEventEmitter`, which
...
def l1_backend_name(config: L1ManagerConfig) -> str:
    """Return the directory backend name for the configured L1 medium.

    Args:
        config: The L1 manager configuration.

    Returns:
        ``"gds"``, ``"devdax"``, or ``"dram"``, by configured medium.
    """
...
class L1DirectoryListener(L1ManagerListener):
    """L1 manager listener that forwards events to a directory emitter.

    Writes (including prefetch completions) map to ``STORE``, manager
    deletions to ``DELETE``, reads and touches to ``ACCESS``;
    reservations are ignored. All callbacks are thread-safe.

    Args:
        emitter: The emitter to forward events to.
        backend: The L1 medium's backend name (see :func:`l1_backend_name`).
    """
    ...
    def _record_store(self, keys: list[ObjectKey], sizes: list[int]) -> None:
    ...
    def on_l1_keys_write_finished(
        ...
        """Record a ``STORE`` event per written key."""
        self._record_store(keys, sizes)

    def on_l1_keys_finish_write_and_reserve_read(
        self, keys: list[ObjectKey], sizes: list[int]
    ) -> None:
        """Record a ``STORE`` event per prefetched key."""
        self._record_store(keys, sizes)

    def on_l1_keys_deleted_by_manager(self, keys: list[ObjectKey]) -> None:
        """Record a ``DELETE`` event per deleted key (evictions included)."""
        self._emitter.record(
...
class L2DirectoryListener(L2AdapterListener):
