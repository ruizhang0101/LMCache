...
async def lifespan(app: FastAPI):
    ...
    # Optionally report cache events to the coordinator: an L1 listener
    # plus one listener per L2 adapter tag events with their tier and
    # backend name; a shared emitter sequences and flushes them through
    # the transport sink (HTTP today; a message-queue sink can replace
    # it without touching the listeners). On the coordinator the stream
    # feeds the key directory and, through its event router, the
    # usage/eviction consumers.
    directory_event_task = None
    if (
        coordinator_client is not None
        and coordinator_config.url
        and coordinator_config.event_reporting
    ):
        directory_event_emitter = CacheEventEmitter(
        ...
        if engine.storage_manager is not None:
            engine.storage_manager.register_l1_listener(
                L1DirectoryListener(
                    directory_event_emitter,
                    backend=l1_backend_name(
            ...
            for descriptor, adapter in engine.storage_manager.l2_adapters():
                adapter.register_listener(
                    L2DirectoryListener(
                        directory_event_emitter, backend=descriptor.type_name
                    )
                )
        directory_event_task = asyncio.create_task(directory_event_emitter.run())

    app.state.coordinator_client = coordinator_client
    app.state.coordinator_registration_task = coordinator_registration_task
    app.state.directory_event_task = directory_event_task
    ...
    directory_event_task = getattr(app.state, "directory_event_task", None)
    if directory_event_task is not None:
        directory_event_task.cancel()
        with contextlib.suppress(asyncio.CancelledError):
            await directory_event_task
